...
public class SnmpTreeConstructor
{
	...
	if (!info.oidVisitedMap.contains(oidStack))
	{
		...
		if (inTable)
		    parent = buildStringPath(tableNameStack, true)
			    + buildStringPath(inTableNameStackStack.peek(),
				    true);
		else
		    parent = buildStringPath(nameStack, false);

		if (info.isSimple)
		    mc.addItem(parent, name, oid, info.field.getType(),
    ...
    /**
     * Checks to see if descending into the specified object will cause a
     * circular loop to occur.
     * 
     * @param obj
